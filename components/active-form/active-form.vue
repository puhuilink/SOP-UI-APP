                ...
                <view class="form-item flex-row--c">
                    ...
                    <view class="line" v-else-if="item.__config__.tag == 'el-select'">
                        ...
                        <view class="line-right pr20">
                            ...
                            <u-action-sheet :actions="item.__slot__.options" :title="item.placeholder"
                                            safeAreaInsetBottom
                                            cancelText="取消"
                    ...
                    <!-- 级联选择-->
                    <view class="line" v-else-if="item.__config__.tag == 'el-cascader' ||  item.__config__.tag === 'el-date-range' ||  item.__config__.tag === 'el-date'">
                        <view :class="item.__config__.required ? 'line-left' : 'p-l14 line-left'">
                            <text class="colorRed" v-if="item.__config__.required">*</text>
                            <text class="num" v-if="num">{{ index + 1 }}.</text>
                            {{ item.__config__.label }}：
                    ...
                    <view class="line" v-else-if="item.__config__.tag == 'el-rate'">
                        ...
                        <view class="line-right pr20">
                         <u-rate :count="item.max" v-model="form[item.formDataType]" active-icon="heart-fill" inactive-icon="heart"></u-rate>
                        </view>
                    </view>
                    <!-- 多行文本框 -->
                    ...
                    <!-- 多选框 -->
                    <view class="line-col" v-else-if="item.__config__.tag == 'el-checkbox'">
                        <view :class="item.__config__.required ? 'line-left' : 'p-l14 line-left'">
                            <text class="colorRed" v-if="item.__config__.required">*</text>
                            <text class="num" v-if="num">{{ index + 1 }}.</text>
                            {{ item.__config__.label }}：
...
export default {
    ...
    methods: {
        ...
        afterRead($event, item) {
            ...
            item.__config__.regList = item.auto-upload ? $event.upload : [$event.upload]
            this.$emit('input', this.fields)
        },
        //显示select
        showSelect(item) {
                    ...
                    if (item.__config__.required) {
                        switch (item.tag) {
                            ...
                            case "mobile":
                                ...
                                break;
                            default:
                                if ([null, undefined, ''].includes(this.form[item.formDataType])) {
                                    uni.showToast({
